...
contextBridge.exposeInMainWorld('api', {
  ...
  onLoginStatus: (callback: (status: { spotify?: boolean }) => void) =>
  ...
  selectPlaylist: (playlistId: string) =>
    ipcRenderer.invoke('select-playlist', playlistId),

  getPlaylists: () => ipcRenderer.invoke('get-playlists'),

  getTracks: (playlistId: string) =>
    ipcRenderer.invoke('get-tracks', playlistId),

  deletePlaylist: (index: number) =>
    ipcRenderer.invoke('delete-playlist', index),

  reloadPlaylist: (index: number) =>
    ipcRenderer.invoke('reload-playlist', index),
  ...
  getTrackInfo: (trackId: string) =>
    ipcRenderer.invoke('get-track-info', trackId),

  getCurrentlyPlaying: () => ipcRenderer.invoke('get-currently-playing'),

  saveKeybind: (index: number, keybind: string) =>
    ipcRenderer.invoke('save-keybind', index, keybind),

  onPlaylistUpdated: (callback: (playlistIndex: number) => void) => {
    ipcRenderer.on('playlist-updated', (_, playlistIndex) => callback(playlistIndex));
  },

  onTrackAlreadyExists: (callback: (playlistIndex: number, trackName: string) => void) => {
    ipcRenderer.on('track-already-exists', (_, playlistIndex, trackName) =>
      callback(playlistIndex, trackName)
    );
  },
});
